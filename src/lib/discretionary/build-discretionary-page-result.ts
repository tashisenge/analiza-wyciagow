...
function limitUsedPercent(summaryTotal: number, monthlyLimit: number | null): number | null {
...
export function buildDiscretionaryPageResult(input: {
  ...
  return {
    ...
    coveragePercent: expenseCoveragePercent(input.periods.current, input.periods.pairedKeys),
  };
}
